...
| 16 | Fantômes et hypothèse locale | En ligne, le client reconstruit une position depuis sa seule vue pour afficher les destinations (`hypothesis.ts`). Les **fantômes en sont exclus** : un souvenir peut être périmé, et l'ériger en obstacle masquerait des coups réellement jouables. L'hypothèse est donc volontairement optimiste — elle propose un sur-ensemble des coups légaux, et le serveur arbitre. Le sens de l'erreur est un choix : mieux vaut un refus expliqué qu'un coup escamoté. | `apps/web/src/game/hypothesis.ts` |
